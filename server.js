...
io.on("connection", (socket) => {
  ...
  socket.on("user_joined_room", ({ username, room }) => {
    ...
    socket.on("ask_start_game", (settings) => {
      // Get ten questions from api

      fetch(
        `https://opentdb.com/api.php?amount=${settings.num_of_questions}&type=multiple&category=${settings.category_selected}`
      )
        .then((res) => {
          return res.json()
        })
