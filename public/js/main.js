...
const questions_select = document.getElementById("questions_select")
const startgame_btn = document.getElementById("startgame_btn")

let time_selected = Number(time_select.value) * 1000
let num_of_questions = Number(questions_select.value)
...
time_select.addEventListener("change", (e) => {
  time_selected = Number(e.target.value) * 1000
})

questions_select.addEventListener("change", (e) => {
  num_of_questions = Number(e.target.value)
  document.querySelector(
    ".num_of_questions"
  ).textContent = ` / ${num_of_questions}`
})
...
join_btn.addEventListener("click", function () {
  // Get info
  username =
    username_input.value.charAt(0).toUpperCase() + username_input.value.slice(1)
...
startgame_btn.addEventListener("click", async function () {
  ...
  socket.emit("ask_start_game", {
    num_of_questions,
    category_selected: category_select.value,
  })
  ...
  setTimeout(async () => {
    ...
    while (index < num_of_questions) {
      // Ask for question
      socket.emit("ask_question", index)
      await sleep(time_selected - 2000)
...
socket.on(
  "display_question",
  ({ index, difficulty, category, question, all_answers, error }) => {
    if (error) {
      alert(error)
...
// Display results
socket.on("display_results", ({ correct_answer, error }) => {
  if (error) {
    window.location.reload()
    return
  }
...
function submitChoice(choice_id) {
  // Show choice
